...
-   Making conclusions and recommendations based on the findings gotten.
...
> From the correlation matrix we can deduce the following: Variables
> that are positively correlated are:
>
...
>
> -   Age and daily time spent on site \#\#\# Loading the tidyverese
...
> Based on the analysis: - When consumers spend more on internet usage
> doesn't necessary mean that they will click on the ad
...
more likely to spend less time on the site. \# Supervised Modelling
...
```{r}
#using label encoding to transform categorical variables for Ad.Topc=ic.Line,City and Country
library(CatEncoders)

  encode <- LabelEncoder.fit(advertising$Ad.Topic.Line)
advertising$Ad.Topic.Line <- transform(encode, advertising$Ad.Topic.Line)

 encode <- LabelEncoder.fit(advertising$City)
  advertising$City <- transform(encode, advertising$City)
unique(advertising$City)

encode <- LabelEncoder.fit(advertising$Country)
  advertising$Country <- transform(encode, advertising$Country)
unique(advertising$Country)
```

```{r}
advertising$Ad.Topic.Line <- as.numeric(advertising$Ad.Topic.Line)
advertising$City <- as.numeric(advertising$City)
...
```{r}
#finding the distribution of the values within the target variable
table(advertising$Clicked.on.Ad)
```

There is no class imbalance within the dataset

```{r}
#loading the necessary libraries for random fores
...
```{r}
ind <- sample(2,nrow(advertising),replace=TRUE,prob =c(0.7,0.3))

train <- advertising[ind ==1,]#70%
test <- advertising[ind==2,]#30%
set.seed(222)
rf <- randomForest(Clicked.on.Ad~.,data=advertising)
print(rf)
```

-   *No of variables at each split is 3*

-   The out of Bag error rate is 3.5% which means it achieved an
    accuracy of approximately 96%

-   The class errors for both clicked and not clicked on add is really
    small, with 484 true negatives and 481 true positives.

-   The number of trees is 500, which is at default settings if not
    specified.
...
```{r}
print(head(train$Clicked.on.Ad))
print(head(test$Clicked.on.Ad))
```

The comparison between prediction and train and test sets affirms
accurate predictions

## Tuning the random Forest model:finding optimal mtrys and ntrees
...
The error rate reduces at around 100, beyond which adding more trees
doesn't decrease the error rate at a substantial amount

## Tuning the mtry

```{r}
tuneRF(train[,-10],train[,10],stepFactor = 0.5,plot=TRUE,ntree=100,trace=TRUE,improve=0.05)
```

maximum number of mtrys is 6, since the 0ut of Bag error decreases
significantly to 6 then rise with more mtrys

## After hyper-parameter tuning

```{r}
rf <- randomForest(Clicked.on.Ad~.,data=train,ntree=100,mtry=6,importance=TRUE,proximity=TRUE)
```

## Variable Importance

```{r}
varImpPlot(rf,sort=T,main="Order of variable importance")
```

```{r}
partialPlot(rf,train,Daily.Time.Spent.on.Site,"1")
```

```{r}
partialPlot(rf,train,Daily.Internet.Usage,"1")
```

\#Both daily internet usage and daily time spent on site are able to
classify some of the 1s up-to a certain point where the graph drops
exponentially

```{r}
#Multidimensional Scaling plot using random forest model
MDSplot(rf,train$Clicked.on.Ad)
```
...
variables mostly used is Daily Time Spent on Site with 470 observations.
Gender had the least number of observations used with only 7.

## Conclusion

Individuals that are likely to click on the ad is one who :

-   spends most of their daily time spent on site

-   highest daily internet usage

-   Higher area income

-   However, Gender doesn't play a role in determining if the client
